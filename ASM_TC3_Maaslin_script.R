...
metadata<-read.table("TC3_metadatav2.txt", header = TRUE, sep = "\t",
                     row.names = 1, stringsAsFactors = FALSE, check.names = FALSE)

data<-read.csv(file = "TC3_D0vsD3-frequency-table-L7F.csv", header = TRUE, sep = ",",
               row.names = 1, stringsAsFactors = FALSE, check.names = FALSE)
...
fit_data<-Maaslin2(
  ...
  output = "Maaslin2_Results_TC3_D0vsD3NONE",
...
data<-read.csv(file = "TC3_D3vsD7-frequency-table-L7F.csv", header = TRUE, sep = ",",
               row.names = 1, stringsAsFactors = FALSE, check.names = FALSE)
...
fit_data<-Maaslin2(
  input_data = data,
  input_metadata = metadata,
  output = "Maaslin2_Results_TC3_D3vsD7NONE",
...
data<-read.csv(file = "TC3_D7vsD10-frequency-table-L7F.csv", header = TRUE, sep = ",",
               row.names = 1, stringsAsFactors = FALSE, check.names = FALSE)
...
fit_data<-Maaslin2(
  input_data = data,
  input_metadata = metadata,
  output = "Maaslin2_Results_TC3_D7vsD10NONE",
  fixed_effects = c("D0vsD3"),
  reference = c(("D7,D10")),
  normalization = "CLR",
  max_significance = 0.05,
...
data<-read.csv(file = "TC3_Day-frequency-table-L7F.csv", header = TRUE, sep = ",",
               row.names = 1, stringsAsFactors = FALSE, check.names = FALSE)
...
fit_data<-Maaslin2(
  input_data = data,
  input_metadata = metadata,
  output = "Maaslin2_Results_TC3_Testing",
  fixed_effects = c("Day"),
...
SVs<-read_qza("TC3_D0vsD3_table.qza")$data
taxonomy<-read_qza("RCM_taxonomy.qza")$data
