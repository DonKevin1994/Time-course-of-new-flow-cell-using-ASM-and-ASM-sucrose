...
TC3_EBC1_df <- as.data.frame(sample_data(physeq))

# Now to look at the library size 

TC3_EBC1_df$LibrarySize<-sample_sums(physeq)
...
TC3_EBC1_df <- TC3_EBC1_df[order(TC3_EBC1_df$LibrarySize),]
TC3_EBC1_df$Index <- seq(nrow(TC3_EBC1_df))
ggplot(data= TC3_EBC1_df, aes(x=Index, y=LibrarySize, color=EBC1vsControl2vsBiological)) +geom_point()
...
TC3_EBC1_contamdf.prev <- isContaminant(physeq, method="prevalence", neg="is.neg", threshold=0.55)

ggplot(data = TC3_EBC1_contamdf.prev, aes(x=p)) +
...
TC3_EBC1_df.pa <- data.frame(pa.pos=taxa_sums(physeq.pa.samples), pa.neg=taxa_sums(physeq.pa.controls), contaminant=TC3_EBC1_contamdf.prev$contaminant)

ggplot(data=TC3_EBC1_df.pa, aes(x=pa.neg, y=pa.pos, color=contaminant)) + geom_point() + xlab("Prevalence (Negative Controls)") + ylab("Prevalence (Samples)") 

#write frequency table to file 

write.table(TC3_EBC1_contamdf.prev, file = "/mnt/c/Users/Kevin/QIIME_workflow/OMT/TC3/frequency_table_TC3_EBC1.csv",
...
TC3_EBC1_contamdf.prev <- tibble::rownames_to_column(TC3_EBC1_contamdf.prev, "featureID")

# Join tables to find taxonomy of contaminants
contaminants_taxonomy <- dplyr::semi_join(taxonomy, TC3_EBC1_contamdf.prev, by = "featureID")

# Now you can view the contaminant taxa and save this as a file to open in Excel if you want.

write.table(contaminants_taxonomy, file = "/mnt/c/Users/Kevin/QIIME_workflow/OMT/TC3/frequency_table_TC3_EBC1_Taxa.csv",
