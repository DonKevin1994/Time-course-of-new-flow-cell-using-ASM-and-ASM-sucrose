...
metadata<-read_tsv("TC2_metadatav2.txt")

ASVs <- read_qza("TC2_table.qza")

greengenes_taxonomy <- read_qza("OMT_data_merged_taxonomy.qza")

insertion_tree <- read_qza("OMT_data_merged_rooted-tree.qza")
...
TC2_EBC1_df <- as.data.frame(sample_data(physeq))

# Now to look at the library size 

TC2_EBC1_df$LibrarySize<-sample_sums(physeq)

# We want to visualise the library size to make sure most of the EBCs have a lower seq count (just do EBC1's first, do core metrics. Do EBC2's after)

TC2_EBC1_df <- TC2_EBC1_df[order(TC2_EBC1_df$LibrarySize),]
TC2_EBC1_df$Index <- seq(nrow(TC2_EBC1_df))
ggplot(data= TC2_EBC1_df, aes(x=Index, y=LibrarySize, color=EBC1vsControl2vsBiological)) +geom_point()
...
sample_data(physeq)$is.neg <- sample_data(physeq)$EBC1vsControl2vsBiological == "EBC1"

TC2_EBC1_contamdf.prev <- isContaminant(physeq, method="prevalence", neg="is.neg", threshold=0.45)

ggplot(data = TC2_EBC1_contamdf.prev, aes(x=p)) +
...
physeq.pa.controls <- prune_samples(sample_data(physeq.pa)$EBC1vsControl2vsBiological == "EBC1", physeq.pa)

physeq.pa.samples <- prune_samples(sample_data(physeq.pa)$EBC1vsControl2vsBiological == "Biological", physeq.pa)

TC2_EBC1_df.pa <- data.frame(pa.pos=taxa_sums(physeq.pa.samples), pa.neg=taxa_sums(physeq.pa.controls), contaminant=TC2_EBC1_contamdf.prev$contaminant)

ggplot(data=TC2_EBC1_df.pa, aes(x=pa.neg, y=pa.pos, color=contaminant)) + geom_point() + xlab("Prevalence (Negative Controls)") + ylab("Prevalence (Samples)") 

#write frequency table to file 

write.table(TC2_EBC1_contamdf.prev, file = "/mnt/c/Users/Kevin/QIIME_workflow/OMT/TC2/frequency_table_TC2_EBC1.csv",
...
library(qiime2R)
taxonomy <- read_qza("OMT_data_merged_taxonomy.qza")
...
TC2_EBC1_contamdf.prev <- tibble::rownames_to_column(TC2_EBC1_contamdf.prev, "featureID")

# Join tables to find taxonomy of contaminants
contaminants_taxonomy <- dplyr::semi_join(taxonomy, TC2_EBC1_contamdf.prev, by = "featureID")

# Now you can view the contaminant taxa and save this as a file to open in Excel if you want.

write.table(contaminants_taxonomy, file = "/mnt/c/Users/Kevin/QIIME_workflow/OMT/TC2/frequency_table_TC2_EBC1_Taxa.csv",
